...
OUT_FEATURES = OUT_DIR / "early_features_10min_v2.csv"
OUT_PRED = OUT_DIR / "expected_win_probs_v2.csv"
...
# New function to count early objectives (dragons and plates) up to 10 minutes, similar structure to kills_diff_up_to_10min but looking for different event types and counting separately for my team and enemy team, then returning the difference in a dict
def objectives_up_to_10min(timeline_json: dict, pid_to_team: dict, my_team: int) -> dict:
    """
    Count early objectives up to 10:00:
    - dragon_kills_diff_10: number of dragons killed by my team minus enemy team
    - plates_diff_10: turret plates taken by my team minus enemy team

    Notes:
    - Dragons show up as ELITE_MONSTER_KILL with monsterType == "DRAGON"
    - Plates often show up as TURRET_PLATE_DESTROYED (killerId indicates who took it)
    """
    frames = timeline_json.get("info", {}).get("frames", [])
    enemy_team = 200 if my_team == 100 else 100
    ...
    for frame in frames:
        for ev in frame.get("events", []):
            ...
            # Dragon kills
            if ev_type == "ELITE_MONSTER_KILL" and ev.get("monsterType") == "DRAGON":
                killer_pid = ev.get("killerId")
                if killer_pid and killer_pid != 0:
                    killer_team = pid_to_team.get(killer_pid)
                    if killer_team == my_team:
                        my_dragons += 1
                    elif killer_team == enemy_team:
            ...
            # Turret plate destroys
            if ev_type == "TURRET_PLATE_DESTROYED":
                killer_pid = ev.get("killerId")
                if killer_pid and killer_pid != 0:
                    killer_team = pid_to_team.get(killer_pid)
                    if killer_team == my_team:
    ...
    return {
        "dragon_kills_diff_10": my_dragons - enemy_dragons,
        "plates_diff_10": my_plates - enemy_plates,
    }
...
def build_features_for_match(match_id: str, my_puuid: str) -> dict:
    ...
    obj = objectives_up_to_10min(timeline_json, pid_to_team, my_team) # new line for objectives
    ...
    return {
        ...
        **obj, # include the objective diffs in the returned dict
    }
...
def main():
    ...
    X = df[["gold_diff_10", "xp_diff_10", "cs_diff_10", "kills_diff_10", "dragon_kills_diff_10", "plates_diff_10"]]
    y = df["win"].astype(int)
    ...
    df["p_win_10min_v2"] = p_win

    # Luck summary
    actual_wins = int(y.sum())
    expected_wins = float(df["p_win_10min_v2"].sum())
    ...
    # Write report
    lines = []
    lines.append("S08 — Expected wins from EARLY-game (10 min) features + obj\n")
    ...
    unlucky_games = df[df["win"] == False].sort_values("p_win_10min_v2", ascending=False).head(10)
    lines.append(unlucky_games[["match_id", "p_win_10min_v2", "gold_diff_10", "xp_diff_10", "cs_diff_10", "kills_diff_10", "dragon_kills_diff_10", "plates_diff_10"]].to_string(index=False))

    lines.append("\nTop 10 most 'lucky' wins (low p_win but win):")
    lucky_games = df[df["win"] == True].sort_values("p_win_10min_v2", ascending=True).head(10)
    lines.append(lucky_games[["match_id", "p_win_10min_v2", "gold_diff_10", "xp_diff_10", "cs_diff_10", "kills_diff_10", "dragon_kills_diff_10", "plates_diff_10"]].to_string(index=False))
